...
public class Problems {
    ...
    public List<String> wordBreak(String s, List<String> wordDict) {
        ...
        wordBreakHelp(s, wordDict, result, 0, new ArrayList<>());
        return result;
    }

    public void wordBreakHelp(String s, List<String> wordDict, List<String> result, int index, List<String> current) {
        if (index == s.length()) {
            StringBuilder sb = new StringBuilder();
            current.forEach(e -> sb.append(e + " "));
            result.add(sb.toString().trim());
        }
        for (int i = index + 1; i <= s.length(); i++) {
            if (wordDict.contains(s.substring(index, i))) {
                current.add(s.substring(index, i) + " ");
                wordBreakHelp(s, wordDict, result, i, current);
